...
$list_arr[]="
<tr bgcolor=#C0C0C0><th>Тип оборудования</th><th>Модель</th><th>MAC(1)</th><th>Питание</th>".$add_read_head."</tr>
...
if (isset($_GET['add'])){
    $list_arr[]="
    <tr bgcolor=#C0C0C0><td colspan=4><b>параметры для компьютера:</b></td></tr>";
}

if (isset($_GET['add']) OR (isset($type) AND intval($type)==5)){
    
    if (!empty($pc_id)) {$show_id="<b>ID".$pc_id."</b>";}
    else {$show_id="";}

    $list_arr[]="
<tr><td rowspan=2 align=center>".$show_id."</td><td>AnyDesk</td><td>MAC(2)</td><td>Плеер</td></tr>
<tr><td><input type=text name=anydesk value='".$anydesk."'></td><td><input type=text name=mac2 value='".$mac2."'></td><td>".$player."</td></tr>";
}
